...
window.addEventListener("scroll", () => {
  const windowHiegth1 = window.pageYOffset;
  const sideBarHeigth = barBtn.getBoundingClientRect().height;
  if (windowHiegth1 > sideBarHeigth) {
    barBtn.classList.add("sideBarToggle");
  } else {
    barBtn.classList.remove("sideBarToggle");
  }
});
